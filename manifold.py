...
# Not in use - replaced by Excel plots within spreadsheet generated by XlsxWriter   Novemer 10, 2018
def eigenvector_clusterwise_plots(algorithm, eigenvectors, cluster_labels, diameter, output_dir, timestamp_string):
	...
	if N>2:
		...
		# Actually we can do   	cluster_ids, card_per_cluster = np.unique(cluster_labels, return_counts=True)
		#                       num_clusters = len(cluster_ids)
		# as in generate_eigenvector_spreadsheet()
		...
		for k in range(K):
			print("Cluster " + str(k))
			pointers_clstr = np.asarray([ptr for ptr in pointers_b if cluster_labels[ptr]==k])	# NOTE- how to get elts of cluster
...
def sk_lifted_spectral_clustering(affinity_matrix, num_eigenvectors, num_clusters, kmeans_random_state):
	...
	# I'd rather give it dense affinity.  But try with sparse, since there was an issue needing np.asarray
